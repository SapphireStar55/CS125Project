...
int shiphealth=100;
int Fuel=50;
int Valcegaussskil=5, Brianstunner=2;
int gaussammo=50;
...
int intro() {
	...
	printf("\e[1;34m In the lengthy prose of typical corpo talk, it details");
...
int combat() {
	...
	int pirhealth[4]= {100, 100,100,100};
	...
	while ((partyhealth[0]>0) || (partyhealth[1]>0) || (partyhealth[2]>0) || (partyhealth[3]>0)) {
		...
		if (partyarmor[piratechoice]<=pirattack) {
			...
			printf("\n Valce Health:%d \n Brian Health:%d \n Engie Health:%d \n Dhoerrgh Health:%d", partyhealth[0], partyhealth[1],partyhealth[2], partyhealth[3]);
		}
		else{
		    printf("\nThe pirate missed %d", partyarmor[piratechoice]);
		}
	}
